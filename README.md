# Demyx discord bot
Bot de discord voltado para musica construido usando Javascript e a biblioteca discord.js.
# Funcionalidades implementadas
!play : recebe um link do youtube ou pode-se pesquisar por um video e começa a tocar o audio do video no canal de voz, tambem é possivel adicionar uma playlist do youtube para a reprodução usando o link da playlist, e se ja estiver tocando adiciona uma musica para o final da fila de reprodução. <br>
!skip : pula para a proxima musica da lista. <br>
!stop : desconecta o bot do servidor e limpa a lista de reprodução. <br>
!now : mostra a musica tocando no momento. <br>
!queue : mostra fila de reprodução.<br>
!pause : pausa a musica atual.<br>
!resume : retoma a musica pausada.<br>
!loop : coloca a playlist em loop. <br>
